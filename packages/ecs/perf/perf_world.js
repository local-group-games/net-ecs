...
module.exports.run = function run() {
  ...
  const positionAndVelocity = query(Position, Velocity)
  const positionChanged = query(Position).filter(changed(), tags)
  const systems = [
    (data, { mut, storage }) => {
      for (let [p, v] of positionAndVelocity.run(storage)) {
        c++
        if (v.x || v.y) {
          p = mut(p)
          p.x += v.x
    ...
    (data, { storage }) => {
      for (let [] of positionChanged.run(storage)) {
        cc++
      }
    },
  ]
