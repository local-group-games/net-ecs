...
describe("Storage (integration)", () => {
  ...
  it("detects changed entities", () => {
    ...
    const queryA = query(ComponentA).filter(filter)

    expect(Array.from(queryA.run(storage)).length).toBe(2)
    expect(Array.from(queryA.run(storage)).length).toBe(0)

    storage.bump(entities[0])

    expect(Array.from(queryA.run(storage)).length).toBe(1)
    expect(Array.from(queryA.run(storage)).length).toBe(0)
  })
})
