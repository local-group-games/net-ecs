import { Schema, ComponentOfSchema, DataType, $isDataType, SchemaKey } from "./schema_types"

export function createDataType<T>(schema: Omit<DataType<T>, typeof $isDataType>): DataType<T> {
...
export function initializeComponentFromSchema<S extends Schema>(
  ...
  return component as ComponentOfSchema<S>
}

export function resetComponentFromSchema<S extends Schema>(component: any, schema: S) {
  for (const prop in schema) {
    const value = schema[prop] as SchemaKey
